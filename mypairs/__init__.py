...
class all_pairs2(object):
    ...
    def __init__(self, options, filter_func=lambda x: True, previously_tested=[[]], n=2):
        ...
        self.__n = n
        self.__pairs = s.PairsStorage(n)
        self.__max_unique_pairs_expected = get_max_comb_number(options, n)
        self.__working_arr = []

        for i in range(len(options)):
            self.__working_arr.append([item("a%iv%i" % (i, j), value) for j, value in enumerate(options[i])])
    ...
    def resort_working_array(self, chosen_values_arr, num):
        for item in self.__working_arr[num]:
            ...
            for i in range(0, self.__n):
                ...
                new_combs.append(set([s.key(z) for z in xunique_combinations(chosen_values_arr + [item], i + 1)]) - self.__pairs.get_combs()[i])
            # weighting the node
